...
namespace Google.Protobuf.Test
{
    public class JsonEnumvalCustomStringTest
    {
        ...
        [TestCase(Armor.Solleret, "sabaton", "ARMOR_SOLLERET", "ARMOR_SABATON")]
        ...
        public void ParseInvalidValueFails(string jsonValue)
        {
            ...
            Assert.Throws<InvalidProtocolBufferException>(() => JsonParser.Default.Parse<Knight>(json));
        }
        ...
        [TestCase("ARMOR_great_helm")]
        [TestCase("GR8 HELM")]
        public void ParseUnknownString_IgnoreUnknownFields(string unrecognizedJsonValue)
        {
        ...
        public void OriginalNameAttribute(string fieldName, string expectedJsonEnumValueName, string expectedName, bool expectedPreferredAlias)
        {
            var field = typeof(Armor).GetTypeInfo().GetDeclaredField(fieldName);
            Assert.IsNotNull(field, $"Field {fieldName} should exist in Armor enum.");
            var attr = field.GetCustomAttributes<OriginalNameAttribute>().FirstOrDefault();
            Assert.IsNotNull(attr, $"Field {fieldName} should have OriginalNameAttribute.");
