...
std::unique_ptr<ImmutableFieldGenerator> MakeImmutableGenerator(
    const FieldDescriptor* field, int bitIndex, Context* context) {
  if (field->is_repeated()) {
    switch (GetJavaType(field)) {
      case JAVATYPE_MESSAGE:
        if (IsMapEntry(field->message_type())) {
          return std::make_unique<ImmutableMapFieldGenerator>(field, bitIndex,
                                                              context);
        } else {
          return std::make_unique<RepeatedImmutableMessageFieldGenerator>(
              field, bitIndex, context);
        }
      case JAVATYPE_ENUM:
      ...
      switch (GetJavaType(field)) {
        case JAVATYPE_MESSAGE:
          return std::make_unique<ImmutableMessageFieldGenerator>(
              field, bitIndex, context);
        case JAVATYPE_ENUM:
          return std::make_unique<ImmutableEnumFieldGenerator>(field, bitIndex,
                                                               context);
        case JAVATYPE_STRING:
...
FieldGeneratorMap<ImmutableFieldGenerator> MakeImmutableFieldGenerators(
  ...
  for (int i = 0; i < descriptor->field_count(); i++) {
    ...
    auto generator = MakeImmutableGenerator(field, bitIndex, context);
    bitIndex += generator->GetNumBits();
